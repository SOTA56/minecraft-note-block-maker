...
export function decodeMidiText(bytes: Uint8Array) {
  if (!bytes.length) return ''
  // The MIDI specification predates Unicode. Modern DAWs usually write UTF-8,
  // while many Japanese applications still emit Shift_JIS/Windows-31J.
  // Decode strict UTF-8 first so malformed bytes do not silently become �,
  // then use the browser's WHATWG Shift_JIS decoder as the compatible fallback.
...
export function parseMidi(buffer: ArrayBuffer): ParsedMidi {
  ...
  if (ascii(read(4)) !== 'MThd') throw new Error('標準MIDIファイル（.mid/.midi）を選択してください。')
  ...
  for (let trackIndex = 0; trackIndex < trackCount; trackIndex++) {
    if (ascii(read(4)) !== 'MTrk') throw new Error('MIDIトラックが不正です。')
    ...
    while (offset < end) {
      ...
      if (status === 0xff) {
        ...
        if (type === 0x03 && data.length) name = decodeMidiText(data).replace(/\0/g, '').trim() || name
