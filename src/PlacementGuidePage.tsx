...
export default function PlacementGuidePage({language,onBack,onHome}:Props){
  ...
  return <main className="placement-page">
    ...
    <div className="placement-content">
      ...
      <section className="placement-section"><h2>{t.layers}</h2><p>{t.layersText.replace('側面や上面にダストをつなぎます。','前のレイヤーの終わりから次のレイヤーの始まりにレッドストーンダストをつなぎます。')}</p><div className="placement-image-grid"><figure><img src="/assets/placement/compact-layer-stack.png" alt="詰め詰め回路のレイヤーを重ねた例"/><figcaption>{t.layersText.replace('側面や上面にダストをつなぎます。','前のレイヤーの終わりから次のレイヤーの始まりにレッドストーンダストをつなぎます。')}</figcaption></figure><figure><img src="/assets/placement/compact-layer-connect.png" alt="階段状にレイヤーをつないだ例"/><figcaption>階段状にブロックをつなぎ、前のレイヤーの終わりから次のレイヤーの始まりへレッドストーンダストをつなぎます。</figcaption></figure></div></section>
    </div>
