...
function App() {
  ...
  const [language, setLanguage] = useState<'ja' | 'en'>(() => navigator.language.toLowerCase().startsWith('ja') ? 'ja' : 'en')
  const [stepHeight, setStepHeight] = useState(30)
  const fileRef = useRef<HTMLInputElement>(null)
  const dragRef = useRef<{ originStep: number; originPitch: number; step: number; pitch: number; moved: boolean } | null>(null)
  const pinchRef = useRef<{ distance: number; height: number } | null>(null)
  ...
  const setDraggedNote = (fromStep: number, fromPitch: number, step: number, pitch: number) => updateTrack({ notes: [...active.notes.filter(n => !(n.step === fromStep && n.pitch === fromPitch) && !(n.step === step && n.pitch === pitch)), { step, pitch }] })
  const handlePointerDown = (event: React.PointerEvent, step: number, pitch: number) => {
    if (event.pointerType === 'mouse') event.currentTarget.setPointerCapture(event.pointerId)
    dragRef.current = { originStep: step, originPitch: pitch, step, pitch, moved: false }
    if (!active.notes.some(n => n.step === step && n.pitch === pitch)) setDraggedNote(step, pitch, step, pitch)
  ...
  return <main className="app">
    <header className="topbar">
      ...
      <div className="top-actions"><button className="language" onClick={() => setLanguage(language === 'ja' ? 'en' : 'ja')}>{language === 'ja' ? 'EN' : '日本語'}</button><div className="status"><span className="live-dot" /> {t.autosaved}</div></div>
    </header>
    ...
    <section className="transport">
      ...
      <div className="meter"><small>{t.delay}</small><strong>1 RT</strong><span>{(2 / project.tickRate).toFixed(3)} SEC</span></div>
      ...
      <div className={`poly ${polyphony > 9 ? 'warn' : ''}`}><small>{t.maxPoly}</small><strong>{polyphony}</strong><span>{t.notes}</span></div>
    </section>
    ...
    <section className="roll" aria-label="縦方向ピアノロール" style={{ '--step-height': `${stepHeight}px` } as React.CSSProperties} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp} onPointerCancel={handlePointerUp}
      onTouchStart={e => { if (e.touches.length === 2) { const d = Math.hypot(e.touches[0].clientX-e.touches[1].clientX,e.touches[0].clientY-e.touches[1].clientY); pinchRef.current={distance:d,height:stepHeight} } }}
      onTouchMove={e => { if (e.touches.length === 2 && pinchRef.current) { const d=Math.hypot(e.touches[0].clientX-e.touches[1].clientX,e.touches[0].clientY-e.touches[1].clientY); setStepHeight(Math.max(22,Math.min(64,pinchRef.current.height*d/pinchRef.current.distance))) } }}>
