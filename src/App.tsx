...
const midiImportNotice:Record<string,{title:string;body:string;cancel:string;continue:string}>={
  ja:{title:'MIDIを読み込む前に',body:'他人が制作・公開したMIDIを読み込む場合は、著作権、ライセンス、利用できる範囲を必ず確認してください。許可なく利用したり、生成した設計図を自分の作品であるかのように公開しないでください。',cancel:'キャンセル',continue:'MIDIファイルを選ぶ'},
  en:{title:'Before importing MIDI',body:'Before importing MIDI created or published by someone else, check its copyright, license, and permitted uses. Do not use it without permission or publish the generated blueprint as if it were your own work.',cancel:'CANCEL',continue:'CHOOSE MIDI FILE'},
  es:{title:'Antes de importar MIDI',body:'Antes de importar un MIDI creado o publicado por otra persona, comprueba sus derechos de autor, licencia y usos permitidos. No lo uses sin permiso ni publiques el plano generado como si fuera obra tuya.',cancel:'CANCELAR',continue:'ELEGIR ARCHIVO MIDI'},
  fr:{title:'Avant d’importer un MIDI',body:'Avant d’importer un MIDI créé ou publié par une autre personne, vérifiez ses droits d’auteur, sa licence et les usages autorisés. Ne l’utilisez pas sans autorisation et ne publiez pas le plan généré comme s’il s’agissait de votre propre création.',cancel:'ANNULER',continue:'CHOISIR UN FICHIER MIDI'},
  de:{title:'Vor dem MIDI-Import',body:'Prüfe vor dem Import einer von anderen erstellten oder veröffentlichten MIDI-Datei das Urheberrecht, die Lizenz und die erlaubte Nutzung. Verwende sie nicht ohne Erlaubnis und veröffentliche den erzeugten Bauplan nicht als dein eigenes Werk.',cancel:'ABBRECHEN',continue:'MIDI-DATEI WÄHLEN'},
  zh:{title:'导入 MIDI 前',body:'导入他人制作或公开的 MIDI 前，请务必确认其著作权、许可协议和允许的使用范围。请勿擅自使用，也不要将生成的设计图冒充为自己的作品发布。',cancel:'取消',continue:'选择 MIDI 文件'},
  'zh-tw':{title:'匯入 MIDI 前',body:'匯入他人製作或公開的 MIDI 前，請務必確認其著作權、授權條款與允許的使用範圍。請勿擅自使用，也不要將產生的設計圖冒充為自己的作品發布。',cancel:'取消',continue:'選擇 MIDI 檔案'},
  ko:{title:'MIDI를 불러오기 전에',body:'다른 사람이 제작하거나 공개한 MIDI를 불러올 때는 저작권, 라이선스와 허용된 이용 범위를 반드시 확인하세요. 허가 없이 사용하거나 생성된 설계도를 자신이 만든 작품처럼 공개하지 마세요.',cancel:'취소',continue:'MIDI 파일 선택'},
  id:{title:'Sebelum mengimpor MIDI',body:'Sebelum mengimpor MIDI yang dibuat atau dibagikan orang lain, periksa hak cipta, lisensi, dan batas penggunaannya. Jangan gunakan tanpa izin atau menerbitkan denah yang dihasilkan seolah-olah karya Anda sendiri.',cancel:'BATAL',continue:'PILIH FILE MIDI'},
}
const createInitialProject = ():Project => ({ format: 'oto-blogic', version: 1, title: 'SONG TITLE', edition: 'java', tickRate: 20, delayUnit:1, steps: 64, tracks: Array.from({ length: 20 }, (_, i) => makeTrack(i)), blueprint:{runLength:16,compactSize:50,compactSplitLayers:true,fold:'right',compactFold:'right',includeSilentEdges:true,repeaterDisplay:'delay',fishboneMode:'auto',fishboneManual:{},fishbonePackColumns:false,fishboneSpatialAudio:false,fishbonePlayerHeight:5} })
...
function App() {
  ...
  const [midiNoticeOpen,setMidiNoticeOpen]=useState(false)
  ...
  const load = async (file?: File) => { if (!file) return; const raw=(await file.text()).replace(/^\uFEFF/,''); const jsonStart=raw.indexOf('{'); if(jsonStart<0)throw new Error('対応していない.obg/.nbmファイルです'); const data = JSON.parse(raw.slice(jsonStart)); if (!['oto-blogic','note-block-maker'].includes(data.format) || data.version !== 1) throw new Error('対応していない.obg/.nbmファイルです'); const migrated = normalizeProject(data);commitProject(()=>migrated);setActiveId(migrated.tracks[0].id);setBarsDraft(String(migrated.steps/16));setBpmDraft(String(Math.round(migrated.tickRate*7.5))) }
  ...
  return <main className={`app ${desktopLayout?'desktop-layout':'touch-layout'}`}>
    ...
    {panel === 'settings' && <div className="drawer settings"><label>{t.trackName}<input value={active.name} autoComplete="off" autoCapitalize="characters" spellCheck={false} onChange={e => updateTrack({ name: e.currentTarget.value })} /></label><label>{t.instrument}<select value={active.instrument} onChange={e => updateTrack({ instrument: e.target.value })}>{INSTRUMENTS.map(x => <option key={x.id} value={x.id}>{language === 'ja' ? x.ja : x.en} — {instrumentBlockName(x,language)}</option>)}</select></label><div className="block-guide"><i className={`block-preview ${instrument.texture}`} /><span><small>{t.block}</small><b>{instrumentBlockName(instrument,language)}</b></span></div><label>{t.volume} <b>{Math.round(active.volume * 100)}</b><input type="range" min="0" max="1" step=".01" value={active.volume} onChange={e => updateTrack({ volume: +e.target.value })} /></label><label>PAN <b>{Math.round(active.pan * 100)}</b><input type="range" min="-1" max="1" step=".01" value={active.pan} onChange={e => updateTrack({ pan: +e.target.value })} /></label><div className="setting-switches"><button className={active.muted ? 'danger' : ''} onClick={() => updateTrack({muted:!active.muted})}>M {c[19]}</button><button className={active.solo ? 'solo' : ''} onClick={() => updateTrack({solo:!active.solo})}>S SOLO</button></div></div>}
    ...
    {midiNoticeOpen&&<div className="midi-import-backdrop" role="presentation" onPointerDown={event=>{if(event.target===event.currentTarget)setMidiNoticeOpen(false)}}><section className="midi-import-dialog midi-import-notice" role="alertdialog" aria-modal="true" aria-labelledby="midi-notice-title" aria-describedby="midi-notice-description"><span className="midi-notice-icon" aria-hidden="true"><i>!</i></span><h2 id="midi-notice-title">{(midiImportNotice[language]??midiImportNotice.en).title}</h2><p id="midi-notice-description">{(midiImportNotice[language]??midiImportNotice.en).body}</p><div className="midi-import-actions"><button onClick={()=>setMidiNoticeOpen(false)}>{(midiImportNotice[language]??midiImportNotice.en).cancel}</button><button className="primary" onClick={()=>{setMidiNoticeOpen(false);midiFileRef.current?.click()}}>{(midiImportNotice[language]??midiImportNotice.en).continue}</button></div></section></div>}
