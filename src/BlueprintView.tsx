...
export default function BlueprintView({project,instruments,language,onBack}:Props){
  ...
  const cell=34,axisLeft=30,axisTop=24
  const applyZoom=(value:number,centerX?:number,centerY?:number)=>{
    ...
    stageRef.current.style.width=`${(axisLeft+plan.width*cell)*next}px`;stageRef.current.style.height=`${(axisTop+plan.height*cell)*next}px`
  ...
  useEffect(()=>{zoomRef.current=1;if(shellRef.current)shellRef.current.style.transform='scale(1)';if(stageRef.current&&plan){stageRef.current.style.width=`${axisLeft+plan.width*cell}px`;stageRef.current.style.height=`${axisTop+plan.height*cell}px`}if(zoomTextRef.current)zoomTextRef.current.textContent='100%'},[plan?.width,plan?.height])
  ...
  const pointerMove=(event:React.PointerEvent<HTMLElement>)=>{
    ...
    if(gesture.pointers.size>=2&&plan&&shellRef.current&&stageRef.current){const[a,b]=[...gesture.pointers.values()];const cx=(a.x+b.x)/2,cy=(a.y+b.y)/2;const next=Math.max(.45,Math.min(2.4,gesture.startZoom*Math.hypot(a.x-b.x,a.y-b.y)/gesture.startDistance));zoomRef.current=next;shellRef.current.style.transform=`scale(${next})`;stageRef.current.style.width=`${(axisLeft+plan.width*cell)*next}px`;stageRef.current.style.height=`${(axisTop+plan.height*cell)*next}px`;event.currentTarget.scrollLeft=gesture.anchorX*next-cx;event.currentTarget.scrollTop=gesture.anchorY*next-cy;if(zoomTextRef.current)zoomTextRef.current.textContent=`${Math.round(next*100)}%`}
  ...
  return <main className="blueprint-page">
    ...
    {kind==='easy'?<>
      ...
      {!eligible?<section className="blueprint-warning"><b>!</b><div><strong>{ja?'かんたん回路では生成できません':'NOT AVAILABLE FOR EASY CIRCUIT'}</strong><p>{ja?`同時発音が最大${poly}音あります。3音以内に減らすか、詰め詰め回路を使用してください。`:`Maximum polyphony is ${poly}. Reduce it to 3 or use Compact Circuit.`}</p></div></section>:plan&&<>
        ...
        <section ref={wrapRef} className="blueprint-board-wrap" onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerEnd} onPointerCancel={pointerEnd}><div ref={stageRef} className="blueprint-stage" style={{width:axisLeft+plan.width*cell,height:axisTop+plan.height*cell}}><div ref={shellRef} className="blueprint-grid-shell" style={{width:axisLeft+plan.width*cell,height:axisTop+plan.height*cell}}><div className="column-labels" style={{left:axisLeft,gridTemplateColumns:`repeat(${plan.width},${cell}px)`}}>{Array.from({length:plan.width},(_,index)=><b key={index}>{index+1}</b>)}</div><div className="row-labels" style={{top:axisTop,gridTemplateRows:`repeat(${plan.height},${cell}px)`}}>{Array.from({length:plan.height},(_,index)=><b key={index}>{index+1}</b>)}</div><div className="blueprint-board" style={{left:axisLeft,top:axisTop,width:plan.width*cell,height:plan.height*cell,gridTemplateColumns:`repeat(${plan.width},${cell}px)`,gridTemplateRows:`repeat(${plan.height},${cell}px)`}}>{plan.cells.map((item,index)=><div key={`${item.x}-${item.y}-${index}`} className={`bp-cell ${item.type} ${item.texture??''}`} style={{gridColumn:item.x+1,gridRow:item.y+1}}>{item.type==='repeater'?<><i className={`arrow ${item.direction}`}>➜</i><b>{item.delay}</b></>:item.type==='dust'?<i className="dust-dot"/>:item.type==='note'?<b>{item.label}</b>:item.type==='source'?<b>START</b>:null}</div>)}</div></div></div><div className="blueprint-zoom" onPointerDown={event=>event.stopPropagation()}><button onClick={()=>applyZoom(zoomRef.current-.15)}>−</button><em ref={zoomTextRef}>100%</em><button onClick={()=>applyZoom(zoomRef.current+.15)}>＋</button></div></section>
        <section className="blueprint-legend"><span><i className="legend-note">12</i>{ja?'背景＝下のブロック／数字＝クリック数':'TEXTURE = BLOCK / NUMBER = CLICKS'}</span><span><i className="legend-repeater">➜1</i>{ja?'矢印＝向き／数字＝遅延':'ARROW = DIRECTION / NUMBER = DELAY'}</span><span><i className="legend-rest placeholder"/>{ja?'赤紫色のコンクリート＝仮ブロック':'MAGENTA CONCRETE = PLACEHOLDER'}</span><span><i className="legend-dust"><u/></i>{ja?'赤点＝レッドストーンダスト1個':'RED DOT = ONE DUST'}</span></section>
      </>}
