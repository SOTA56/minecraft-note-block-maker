...
export default function BlueprintView({project,instruments,language,onBack,onSettingsChange}:Props){
  ...
  return <main className={`blueprint-page theme-${theme}`}>
    <header className="blueprint-header"><div className="blueprint-brand"><img className="brand-icon" src="/assets/branding/oto-blogic-icon.png" alt="OTO BLOGIC"/><div><img className="brand-logo" src="/assets/branding/oto-blogic-logo.png" alt="OTO BLOGIC"/><h1>{project.title}</h1></div></div><strong>{ja?'回路設計図':'CIRCUIT BLUEPRINT'}</strong><button className="editor-back" onClick={onBack}><i>←</i><span>{ja?'打ち込みへ':'EDITOR'}</span></button></header>
