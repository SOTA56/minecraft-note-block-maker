...
export default function BlueprintView({project,instruments,language,onBack}:Props){
  ...
  return <main className="blueprint-page">
    ...
    {kind==='easy'?<>
      <section className="blueprint-controls"><label><span>{ja?'折り返しまで':'RUN LENGTH'}</span><input type="text" inputMode="numeric" value={runLengthDraft} onChange={event=>setRunLengthDraft(event.target.value.replace(/[^0-9]/g,''))} onBlur={event=>commitRunLength(event.currentTarget.value)} onKeyDown={event=>{if(event.key==='Enter')event.currentTarget.blur()}}/><em>{ja?'マス':'BLOCKS'}</em></label><div className="fold-control"><span>{ja?'折り返す方向':'FOLD TOWARD'}</span><div className="fold-toggle"><button className={fold==='left'?'active':''} onClick={()=>setFold('left')}>← {ja?'左':'LEFT'}</button><button className={fold==='right'?'active':''} onClick={()=>setFold('right')}>{ja?'右':'RIGHT'} →</button></div></div></section>
