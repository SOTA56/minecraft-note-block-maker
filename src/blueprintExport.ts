...
async function render(plan:Plan,theme:'dark'|'light',legendBlocks:ExportLegendBlock[],ja:boolean){
  ...
  const hasSource=plan.cells.some(item=>item.type==='source'),hasLayerLink=plan.cells.some(item=>item.type==='layer-link')
  const legendItems=legendBlocks.length+3+(hasSource?1:0)+(hasLayerLink?1:0),sideWidth=280,sideLegendHeight=28+legendItems*52
  ...
  for(const item of plan.cells){const x=axis+item.x*cell,y=axis+item.y*cell,cx=x+cell/2,cy=y+cell/2
    ...
    else if(item.type==='layer-link'){context.strokeStyle='#e51d24';context.lineWidth=5;for(const direction of item.connections??[]){context.beginPath();context.moveTo(cx,cy);context.lineTo(cx,cy+(direction==='down'?cell/2:direction==='up'?-cell/2:0));context.stroke()}context.strokeStyle='#35a9ec';context.lineWidth=2;context.strokeRect(x+2,y+2,cell-4,cell-4);context.fillStyle='#35a9ec';context.font='30px "Archivo Black"';context.fillText(item.label??(item.direction==='up'?'↑':'↓'),cx,cy)}
  }
