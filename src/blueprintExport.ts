...
async function render(plan:Plan,theme:'dark'|'light'){
  ...
  context.fillStyle=dark?'#c7cec8':'#333';context.font='12px "Archivo Black"';context.textAlign='right';context.fillText('OTO BLOGIC  Powered by SOTA56',width-axis,axis+plan.height*cell+axis+footer/2)
  return canvas
}

export async function exportBlueprint(plan:Plan,format:'png'|'pdf',title:string,theme:'dark'|'light'){
  const canvas=await render(plan,theme),name=safeName(title)
